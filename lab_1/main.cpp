...
public:
	...
	~ComplexNum()
	{
		DBGPRINT("~ComplexNum (" << this << "): объект удален\n");
	}
};
...
public:
	void init(void)
	{
		...
		while (true)
		{
			...
			{
				ComplexNum *obj = new ComplexNum();
				cout << "Для тестирования функции вывода необходимо создать новый объект класса" << endl;
				cin >> obj;
				cout << "Вывод: " << obj << endl;
				delete obj;
				PAUSE;
			...
			else if (choice == 7)
			{
				ComplexNum *obj = new ComplexNum();
				cin >> obj;
				cout << obj->get_real() << endl
					 << obj->get_imaginary() << endl;
				delete obj;
				PAUSE;
			}
			else if (choice == 8)
